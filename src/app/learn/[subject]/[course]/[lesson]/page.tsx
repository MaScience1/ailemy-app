...
export async function generateMetadata({
  ...
  const { subject: subjectSlug, course: courseSlug, lesson: lessonSlug } =
    await params;
  ...
  if (!course || course.subject.slug !== subjectSlug || !lesson) {
...
export default async function LessonPage({ params }: { params: Params }) {
  ...
  // Coming-soon lessons get the placeholder variant — different shape, different
  // intent. Only render the full lesson page for live content.
      ...
      <div className="mx-auto w-full max-w-6xl px-6 py-10 sm:px-10 sm:py-16">
        <Breadcrumb
          crumbs={[
            ...
            {
              label: course.name,
              href: `/learn/${subjectSlug}/${courseSlug}`,
            },
            { label: lesson.title },
          ]}
        />
            ...
            {lesson.spec_points.length === 0 ? (
              ...
              <ul className="space-y-3">
                {lesson.spec_points.map((sp) => (
                  <li
                    key={sp.id}
                    className="rounded-lg border border-ink/10 bg-snow p-5 transition-colors hover:border-ink/20"
                  >
...
function NeighbourCard({
  ...
  const href = `/learn/${subjectSlug}/${courseSlug}/${neighbour.slug}`;
  ...
  return (
    <Link
      href={href}
      className={`group/nav flex flex-col gap-3 rounded-lg border border-ink/10 bg-snow p-5 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:border-ink/25 ${alignClass}`}
    >
    ...
    <main className="min-h-screen bg-parchment text-ink">
      <div className="mx-auto w-full max-w-3xl px-6 py-16 sm:px-10 sm:py-24">
        ...
        <div className="mt-12 flex justify-center">
          <Link
            href={`/learn/${subjectSlug}/${courseSlug}`}
            className="inline-flex items-center gap-2 rounded-md border border-ink/15 px-5 py-2.5 text-sm font-medium text-ink transition-colors hover:border-ink/30 hover:bg-ink/[0.04]"
          >
